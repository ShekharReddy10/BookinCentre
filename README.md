...
Order matters: database first, then backend (needs the DB URL), then
frontend (needs the backend URL), then wire CORS back.

### 1. Database → Supabase

1. Sign up at [supabase.com](https://supabase.com), **New Project** (pick a
   strong DB password, any region close to you), wait ~2 min for provisioning.
...
auto-configure the service instead of you clicking through settings by hand.

1. Sign up at [render.com](https://render.com), connect your GitHub account,
   authorize the `bookingcentre` repo.
2. **New → Blueprint**, pick this repo — Render reads `render.yaml` and
   proposes the `bookingcentre-backend` service. Click **Apply**.
3. It'll ask you to fill in the env vars marked `sync: false` — at minimum:
   - `DATABASE_URL` → the Supabase connection string from step 1
   - `FRONTEND_ORIGIN` → leave as `http://localhost:3000` for now, you'll
     update it after deploying the frontend
   - Telegram/Resend/SMTP vars → optional, fill in now or later
   - `SECRET_KEY` and `AUTOMATION_SECRET` are auto-generated by the blueprint
4. Deploy. Once live, note the URL (e.g. `https://bookingcentre-backend.onrender.com`).
5. **Test**: `curl https://bookingcentre-backend.onrender.com/health` should
   return `{"status":"ok"}`.
6. **Create your first real admin** via Render's Shell tab (Dashboard → your
   service → Shell): `python -m app.create_user --name "You" --email you@example.com --role admin`

### 3. Frontend → Vercel

1. Sign up at [vercel.com](https://vercel.com), **Add New → Project**,
   import the `bookingcentre` repo.
...
3. Add environment variable `NEXT_PUBLIC_API_URL` = your Render backend URL
   from step 2.
4. Deploy. Note the resulting URL (e.g. `https://bookingcentre.vercel.app`).

### 4. Wire CORS back

Go back to Render → your service → Environment, update `FRONTEND_ORIGIN` to
your actual Vercel URL, save (triggers a redeploy). Without this, the deployed
frontend's API calls will be blocked by CORS.

### 5. Automation → GitHub Actions

`.github/workflows/daily-automation.yml` is ready — in your GitHub repo,
**Settings → Secrets and variables → Actions**, add:
- `BACKEND_URL` = your Render backend URL
- `AUTOMATION_SECRET` = same value Render generated/you set in step 2

It then runs iCal sync + daily reminders automatically. See "Setting up
notifications & channel sync" above for the Telegram/email/iCal parts
themselves.
